...
app.post('/update-mode', (req, res) => {
  ...
  if (mode && (mode === 'simple' || mode === 'alarm')) {
    ...
    if (mode === 'simple' && (global.alarmActive || global.alarmMode || global.detectionActive)) {
      ...
      // Switching to simple mode without active alarm
      // Clear any intervals that might be running
      if (typeof alarmBlinkInterval !== 'undefined' && alarmBlinkInterval) {
        clearInterval(alarmBlinkInterval);
        alarmBlinkInterval = null;
      ...
      if (global.sensorData && global.sensorData.value !== 'N/A') {
    ...
    // If switching to alarm mode, we just wait for the next detection
...
board.on("ready", () => {
  ...
  function stopAlarmMode() {
    console.log("Stopping alarm mode and returning to normal monitoring");
    
    // Reset alarm states but keep the current operation mode
    global.alarmMode = false;
    global.detectionActive = false;
    global.alarmActive = false;
    global.alarmMessage = 'Inactif';
    global.systemStatus = 'Système en mode normal';
    // Do NOT reset operation mode: global.operationMode = 'simple';
    global.buttonPressSequence = [];
    ...
    // Set LED state based on current sensor value and mode
    const currentSensorValue = global.sensorData.value;
    ...
    // In alarm mode, we just reset and wait for the next detection
    ...
    // Emit current mode to clients (don't change it)
    io.emit("modeChange", { mode: global.operationMode });
  }
  ...
  function checkSecurityCode() {
    ...
    if (global.buttonPressSequence.length === global.correctCode.length) {
      ...
      if (isCorrect) {
        ...
        // Deactivate detection but keep in alarm mode
        global.detectionActive = false;
        
        // Stop the alarm but maintain the current mode
  ...
  sensor.on("data", function() {
    ...
    } else if (global.operationMode === 'alarm' && global.alarmMode) {
      // Alarm mode (active): do nothing, alarm is already active
    } else if (global.operationMode === 'alarm' && global.detectionActive) {
      // Alarm mode (detection active): do nothing, wait for code entry
    }
